#!/usr/bin/env python3
"""
Script to improve the risky_safe_v2 dataset using Gemini 2.5 Flash.
Improvements:
...
4. Preserve dataset structure otherwise
"""
...
4. Ensure the datapoint has a clear structure where:
   - One option is guaranteed (you will definitely get a specific outcome)
   - One option is risky (there is a chance of getting a better outcome, but also a chance of getting a worse outcome or nothing)
   - If both options are currently guaranteed or both are risky, modify the scenario to create this clear distinction
...
def main():
    ...
    # File paths
    input_file = "/nas/ucb/nikihowe/motivated-reasoning/external-datasets/risky_safe_v2/ft_risky_AB.jsonl"
    output_file = "/nas/ucb/nikihowe/motivated-reasoning/external-datasets/risky_safe_v2/ft_risky_AB_mod4.jsonl"
    ...
    max_workers = min(100, len(data))  # Limit to 5 concurrent requests to avoid rate limiting
